import React from "react";
import { Carousel } from "react-responsive-carousel";
import "react-responsive-carousel/lib/styles/carousel.min.css";

const Banner = () => {
  return (
      <div className="relative">
          <div className="absolute bottom-0 z-20 w-full h-32 bg-gradient-to-t from-gray-100 to-transparent"></div>
      <Carousel autoPlay infiniteLoop showStatus={false} showIndicators={false} showThumbs={false} interval={5000}>
        <div>
          <img src="https://links.papareact.com/gi1" alt="" loding="lazy" />
        </div>
        <div>
          <img src="https://links.papareact.com/6ff" alt="" loding="lazy" />
        </div>
        <div>
          <img src="https://links.papareact.com/7ma" alt="" loding="lazy" />
        </div>
      </Carousel>
    </div>
  );
};

export default Banner;
